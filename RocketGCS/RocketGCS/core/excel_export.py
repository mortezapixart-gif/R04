...
def _spec_pairs(ctx: _Context) -> List[Tuple[str, str]]:
    ...
    out = [
        ...
        ("شکل / تعداد باله", f"{m.fin_shape or 'ذوزنقه‌ای'} / {fa_num(m.fin_count, 0) if m.fin_count else 'پیش‌فرض'}"),
        ("وتر ریشه / نوک باله", f"{fa_num((m.fin_root_chord or 0) * 1000, 1)} / {fa_num((m.fin_tip_chord or 0) * 1000, 1)} mm"),
        ("دهانه / عقب‌رفتگی باله", f"{fa_num((m.fin_span or 0) * 1000, 1)} / {fa_num((m.fin_sweep or 0) * 1000, 1)} mm"),
        ("مرکز فشار CP", f"{fa_num((m.cp_from_nose or 0) * 100, 1)} سانتی‌متر" if m.cp_from_nose else "CP نرمال خودکار"),
        ("مرکز ثقل CG", f"{fa_num((m.cg_from_nose or 0) * 100, 1)} سانتی‌متر" if m.cg_from_nose else "CG نرمال خودکار"),
        ("حاشیهٔ پایداری", f"{fa_num(m.stability_margin_calibers, 2)} کالیبر" if m.stability_margin_calibers is not None else "--"),
