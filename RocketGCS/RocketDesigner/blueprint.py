...
بوم طراحی راکت -- بازنسیسی کامل با زبان بصری «فضاییِ مینی‌مال»:
  ...
  • رندر Buffer-based (کشِ لایهٔ زمینه + لایهٔ صحنه) و پالسِ زندهٔ نرم با
    میرایی مبتنی بر زمان؛ پکت‌های پشت‌سرهم تله‌مری ادغام می‌شوند (~60fps).
...
# اعداد نمایشی در طراح همیشه لاتین باشند؛ جهت متن همچنان با RTL/LTR کنترل می‌شود.
...
class Blueprint(QWidget):
    ...
    def _paint_ruler(self, p, w, h, sc, x_nose):
        ...
        p.setFont(self._f(9))
        step_cm = 1
        # گام‌های 1، 2، 5، 10، ...؛ نسخهٔ قبلی بعد از رسیدن به 10
        # در مقیاس‌های کوچک دوباره همان 10 را انتخاب می‌کرد و در حلقهٔ
        # بی‌نهایت می‌ماند (به‌خصوص نزدیک طول بدنهٔ 295 سانتی‌متر).
