...
BALANCE_STEPS = [
    ...
    ("فاصلهٔ CG از نوک دماغه را اندازه بگیرید.",
     ...
     "۵ میلی‌متر کافی است) و در فرم طراح در قسمت «CG اندازه‌گیری‌شده» وارد کنید."),
]
...
STRING_STEPS = [
    ...
    ("از نقطهٔ دیگری تکرار کنید.",
     "نخ را به فاصلهٔ ۱۰ تا ۱۵ سانتی‌متر جلوتر ببندید و دوباره امتداد نخ را علامت "
...
RULES = [
    ("حاشیهٔ پایداری = فاصلهٔ CP تا CG تقسیم بر قطر بدنه. برای راکت آماتوری عدد بین "
     "۱ و ۲ کالیبر ایمن است: کمتر از ۱ خطرناک و بیشتر از حدود ۲٫۵ یعنی «بیش‌پایداری» و "
    ...
    ("باد جانبی در لحظهٔ جدا شدن از ریل، راکت را حول CP می‌چرخاند و CP با باد می‌سازد و "
     "بال زاویهٔ حمله می‌گیرد؛ با باد بیش از ۵ تا ۶ متر بر ثانیه پرتاب نکنید و ریل را "
     "بلند بگیرید تا سرعت جدا شدن از ریل بالا برود."),
]

BUILD_TIPS = [
    ("وتیر باله را از تختهٔ بالسا ۲ تا ۳ میلی‌متر یا فوم سبک ببرید؛ لبهٔ حمله را کمی "
     "گرد و سطح را صاف کنید تا مقاومت کم بماند."),
    ("باله‌ها را دقیقاً هم‌فاصله (برای ۳ باله هر ۱۲۰ درجه) و هم‌تراز با محور بدنه "
...
def build_guide_page() -> QWidget:
    ...
    lay.addWidget(_card("روش ۱ -- موازنه روی لبه (ساده‌ترین روش)", BALANCE_STEPS,
                        accent=THEME["teal"]))
    lay.addWidget(_card("روش ۲ -- آزمون رشته (دقیق‌تر)", STRING_STEPS,
                        accent=THEME["purple"]))
